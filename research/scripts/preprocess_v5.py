...
from scripts.config import (
    ...
    COUNTRY_DICT,
    TIMEZONE,
    PREPROCESSING_CONFIG_V5 as PREPROCESSING_CONFIG,
    SECONDS_PER_DAY,
    SECONDS_PER_WEEK,
...
def merge_entsoe_data(
    ...
    print("df_wind_solar_pivot.head()")
    print(df_wind_solar_pivot.head())
    print("df_wind_solar_pivot.info()")
    print(df_wind_solar_pivot.info())
    ...
    print("df_flows_pivot.head()")
    print(df_flows_pivot.head())
    print("df_flows_pivot.info()")
    print(df_flows_pivot.info())
    ...
    print("df_data_pivoted.head()")
    print(df_data_pivoted.head())
    print("df_data_pivoted.info()")
    print(df_data_pivoted.info())

     # Merge with main dataset
    df_data_final = pd.merge(
        df_data_pivoted,
        df_flows_pivot,
        on='datetime',
        how='outer'
    ...
    if output_path:
        ...
        df_data_final.to_csv(out_path, index=False)
...
def missing_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # If there are duplicate timestamps (as seen in loaded CSV),
    # aggregate them by taking the mean to ensure a unique, monotonic index.
    if df_missing.index.has_duplicates:
        df_missing = (
            df_missing
            .groupby(level=0)
...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    high_price_months_str = high_price_months.astype(str)
...
def merge_fuel_prices(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if missing_prices.any():
            ...
            # Fill missing fuel prices with forward fill (use last available values)
            print("Filling missing fuel prices with forward fill...")
            df_merged[price_cols] = df_merged[price_cols].fillna(method='ffill')
            
            # Report remaining missing values after forward fill
            remaining_missing = df_merged[price_cols].isnull().sum()
...
def data_split_classic(
    ...
    # Get validation and test years from config
    val_year = PREPROCESSING_CONFIG['VAL_YEAR']  # 2024
    test_year = PREPROCESSING_CONFIG['TEST_YEAR']  # 2025
    ...
    # Convert future_window_dates to years for initial filtering
    future_window_years = np.array([pd.Timestamp(date).year for date in future_window_dates])
    
    
    # Create initial masks for each split based on years
    train_mask = future_window_years < val_year  # All years before 2024
    val_mask = future_window_years == val_year   # 2024 data
    test_mask = future_window_years == test_year # 2025 data
    
    # Apply initial masks to get the data splits
    X_past_train = past_sequences[train_mask]
    X_future_train = future_sequences[train_mask]
    y_train = targets[train_mask]
    train_past_times = past_times[train_mask]
    train_future_times = future_times[train_mask]
    train_future_window_dates = future_window_dates[train_mask]
    
    X_past_val = past_sequences[val_mask]
    X_future_val = future_sequences[val_mask]
    y_val = targets[val_mask]
    val_past_times = past_times[val_mask]
    val_future_times = future_times[val_mask]
    val_future_window_dates = future_window_dates[val_mask]
    
    X_past_test = past_sequences[test_mask]
    X_future_test = future_sequences[test_mask]
    y_test = targets[test_mask]
    test_past_times = past_times[test_mask]
    test_future_times = future_times[test_mask]
    test_future_window_dates = future_window_dates[test_mask]
    
    # Now apply temporal gap checks to prevent data leakage
    
    # Check validation set: ensure no overlap with training data
    if len(X_past_train) > 0 and len(X_past_val) > 0:
        # Find the last training sequence end time
        last_train_time = train_future_times[-1][-1]  # Last timestamp of last training sequence
        
        # Find valid start of validation set by checking temporal gaps
        val_start_idx = 0
        for i, past_time in enumerate(val_past_times):
            current_time = past_time[0]  # First timestamp of current sequence
            gap_hours = (current_time - last_train_time).total_seconds() / 3600
            if gap_hours >= min_gap_hours:
                val_start_idx = i
                break
        
        # Update validation data to start from the valid index
        if val_start_idx > 0:
            X_past_val = X_past_val[val_start_idx:]
            X_future_val = X_future_val[val_start_idx:]
            y_val = y_val[val_start_idx:]
            val_past_times = val_past_times[val_start_idx:]
            val_future_times = val_future_times[val_start_idx:]
            val_future_window_dates = val_future_window_dates[val_start_idx:]
    
    # Check test set: ensure no overlap with validation data
    if len(X_past_val) > 0 and len(X_past_test) > 0:
        # Find the last validation sequence end time
        last_val_time = val_future_times[-1][-1]  # Last timestamp of last validation sequence
        
        # Find valid start of test set by checking temporal gaps
        test_start_idx = 0
        for i, past_time in enumerate(test_past_times):
            current_time = past_time[0]  # First timestamp of current sequence
        ...
        # Update test data to start from the valid index
        if test_start_idx > 0:
            X_past_test = X_past_test[test_start_idx:]
            X_future_test = X_future_test[test_start_idx:]
            y_test = y_test[test_start_idx:]
            test_past_times = test_past_times[test_start_idx:]
            test_future_times = test_future_times[test_start_idx:]
            test_future_window_dates = test_future_window_dates[test_start_idx:]

    
    # Print detailed split information
    print("\nYear-based split with temporal gap verification:")
    if len(train_future_window_dates) > 0:
        train_years = [pd.Timestamp(date).year for date in train_future_window_dates]
        print(f"Training data years: {sorted(set(train_years))}")
        print(f"Training data range: {min(train_future_window_dates)} to {max(train_future_window_dates)}")
    
    if len(val_future_window_dates) > 0:
        val_years = [pd.Timestamp(date).year for date in val_future_window_dates]
        print(f"Validation data years: {sorted(set(val_years))}")
        print(f"Validation data range: {min(val_future_window_dates)} to {max(val_future_window_dates)}")
    
    if len(test_future_window_dates) > 0:
        test_years = [pd.Timestamp(date).year for date in test_future_window_dates]
        print(f"Test data years: {sorted(set(test_years))}")
        print(f"Test data range: {min(test_future_window_dates)} to {max(test_future_window_dates)}")

    # Print temporal gap information
    print("\nTemporal gap verification:")
    if len(train_future_times) > 0 and len(val_past_times) > 0:
        last_train_time = train_future_times[-1][-1]
        first_val_time = val_past_times[0][0]
        train_val_gap = (first_val_time - last_train_time).total_seconds() / 3600
        print(f"Gap between train-val: {train_val_gap:.1f} hours (required: {min_gap_hours} hours)")
    
    if len(val_future_times) > 0 and len(test_past_times) > 0:
        last_val_time = val_future_times[-1][-1]
        first_test_time = test_past_times[0][0]
        val_test_gap = (first_test_time - last_val_time).total_seconds() / 3600
        print(f"Gap between val-test: {val_test_gap:.1f} hours (required: {min_gap_hours} hours)")

    # Print split information
    print("\nData split summary:")
    print(f"Total sequences: {len(past_sequences):,}")
    ...
    pd.to_pickle(train_past_times, data_model_dir / 'train_past_times.pkl')
    pd.to_pickle(val_past_times, data_model_dir / 'val_past_times.pkl')
    pd.to_pickle(test_past_times, data_model_dir / 'test_past_times.pkl')

    # Save future timestamps using pickle
    pd.to_pickle(train_future_times, data_model_dir / 'train_future_times.pkl')
    pd.to_pickle(val_future_times, data_model_dir / 'val_future_times.pkl')
    pd.to_pickle(test_future_times, data_model_dir / 'test_future_times.pkl')

    # Save future window dates using pickle
    pd.to_pickle(train_future_window_dates, data_model_dir / 'train_future_window_dates.pkl')
    pd.to_pickle(val_future_window_dates, data_model_dir / 'val_future_window_dates.pkl')
    pd.to_pickle(test_future_window_dates, data_model_dir / 'test_future_window_dates.pkl')
...
def main(
    ...
    try:
        ...
        # Clean data
        #df_data = filter_years(df_data, [2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025])
        ...
        df_data['years'] = df_data['datetime'].dt.year
